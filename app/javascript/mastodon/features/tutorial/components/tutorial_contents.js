...
import nicoru from '../../../../images/nicoru.png';
...
const PageOne = ({ page }) => {
  ...
  case 'welcome':
    return (
      <div className='tutorial-back-none'>
        <p className='tutorial-title'>ようこそfriends.nicoへ</p>
        <div className='tutorial-image elephant-friend' />
        <p className='tutorial-paragraph'>friends.nicoはドワンゴが運営するマストドンです。</p>
  ...
  case 'ltl':
    return (
      <div className='tutorial-back-exist'>
        ...
        <p className='tutorial-paragraph'>friends.nico内の公開設定の投稿(トゥート)がすべて流れてきます。</p>
  ...
  case 'nicoru':
    return (
      <div className='tutorial-back-exist'>
        <p className='tutorial-title'><img src={nicoru} alt='nicoru' className='tutorial-nicoru' />ニコる<img src={nicoru} alt='nicoru' className='tutorial-nicoru' /></p>
        <p className='tutorial-paragraph'>トゥートの<img src={nicoruStatus} alt='nicoru' className='tutorial-nicoru' />を押すと、ニコることができます。</p>
        <p className='tutorial-paragraph'>くすっと来たら迷わずニコっちゃいましょう！</p>
      </div>
    );
  case 'toot':
    return (
      <div className='tutorial-back-exist'>
        ...
        <p className='tutorial-paragraph'>みんなが祝ってくれるかもしれません！</p>
      </div>
    );
  default:
    return (
...
const adjustPosition = (contents, targetPosition, direction, balloonPosition) => {
  ...
  case 'right':
    position = {
      top: (targetPosition.top + targetPosition.height * balloonPosition / 100) + 'px',
      bottom: '',
      left: targetPosition.left - 460 - 50 + 'px',
      right: '',
    };
    break;
  }
...
export default class TutorialContents extends React.Component {
  ...
  componentDidMount() {
    ...
    window.onload = () => {
      ['.button--block', '.emojione', '.fa-align-left', '.text-icon-button', '.privacy-dropdown', '.compose-form__buttons'].forEach(query => {
