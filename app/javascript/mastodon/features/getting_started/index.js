...
const messages = defineMessages({
  ...
  niconico: { id: 'navigation_bar.niconico', defaultMessage: 'niconico' },
...
export default class GettingStarted extends ImmutablePureComponent {
  ...
  render () {
    const { intl, myAccount, multiColumn, unreadFollowRequests, columns } = this.props;
    ...
    navItems.push(
      ...
      <ColumnLink key={i++} icon='nicoru--column' text={intl.formatMessage(messages.favourites)} to='/favourites' />,
      <ColumnLink key={i++} icon='list-ul' text={intl.formatMessage(messages.lists)} to='/lists' />,
      <ColumnSubheading key={i++} text={intl.formatMessage(messages.niconico)} />,
      <ColumnLink key={i++} icon='nico' text='ニコニコ形式' href='/nicomment' external />,
      <ColumnLink key={i++} icon='nico' text='ニコニコ動画ランキング' to='/rankings/all' />
    );
    height += 48*2 + 50*3 + 34;

    if (!multiColumn || !columns.find(item => item.get('id') === 'HASHTAG' && item.get('params').get('id') === 'ニコニコ動画タイムライン')) {
      navItems.push(<ColumnLink key={i++} icon='nico' text='動画タイムライン' to='/timelines/tag/ニコニコ動画タイムライン' />);
      height += 50;
    }

    if (!multiColumn || !columns.find(item => item.get('id') === 'HASHTAG' && item.get('params').get('id') === 'ニコニコ生放送タイムライン')) {
      navItems.push(<ColumnLink key={i++} icon='nico' text='生放送タイムライン' to='/timelines/tag/ニコニコ生放送タイムライン' />);
      height += 50;
    }
          ...
          <div className='getting-started__footer'>
            ...
            <p>
              <FormattedMessage
                ...
                values={{ github: <span><a href='https://github.com/dwango/mastodon' rel='noopener' target='_blank'>dwango/mastodon</a> (v{version})</span> }}
              />
            </p>
          </div>
        </div>
